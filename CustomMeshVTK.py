...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        self.smth = np.zeros(shape=int(len(nodes_coords) / 3), dtype=np.double)
        for i in range(len(self.nodes[0])):
            if (-6.5 < self.nodes[0, i] < 6.5) and (-45 < self.nodes[1, i] < -35) and (-20 < self.nodes[2, i] < 12):
                self.smth[i] = 6000
    ...
    def snapshot(self, snap_number):
        ...
        writer.SetFileName("Mario3d-step-" + str(snap_number) + ".vtu")
        writer.Write()
...
print(max(mesh.nodes[0]), min(mesh.nodes[0]))
mesh.snapshot(0)
